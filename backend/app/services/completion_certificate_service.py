...
Renders the document this whole service exists to produce, in the UTA house
style so it sits alongside the university's own paperwork.
...
Fonts: this module registers a TrueType face when one is available rather than
using a standard PDF base font. Helvetica's WinAnsi encoding has no glyphs for
ł, ą, ę, ś, ż, ń or ğ, ş, ı, and this document prints a real student's name and
their host organisation. A name with holes in it on a certificate the registrar
files is not acceptable; transliteration is the fallback, and plain Helvetica
the last resort.
...
# Absolute paths first, bare filenames last. reportlab resolves a bare name
# against its own TTFSearchPath, which does not include the Debian font
# directories — so the container's fonts-dejavu-core install is only found if
# we say where it put them.
_FONT_CANDIDATES = (
    (
        "DejaVuSans",
    ...
    ("DejaVuSansPath", "DejaVuSans.ttf", "DejaVuSans-Bold.ttf"),
)
...
class CertificateService:
    ...
    def create_certificate_pdf(
        ...
        *,
        ...
        signs would be worse than failing loudly.
        """
        blank = [
            label
            for label, value in (
