...
class ReportSubmissionResponse(BaseModel):
    ...
    application_id: str | None = Field(
        default=None,
        description=(
            "Optional, caller-supplied, opaque. A system that already tracks the "
            "placement can pass its own key here and later fetch every attempt "
            "made against it. This service never resolves it, and a submission "
            "without one is reviewed exactly the same way."
        ),
    )
