...
A student emails three PDFs at the end of a placement, and this decides whether
they finished it — issuing the completion certificate the registrar acts on. The service reads them, checks them against each
...
### A model outage cannot hold a package

The verdict comes from counting dates and reading signature fields, so losing
the model costs the coordinator some commentary and changes nothing about who
gets a certificate. **A package is never held because an API was down.**

Systems where the model produces the score do not get that for free: there, an
outage means no real evaluation happened, and the submission has to wait for a
human before anything can be said about it.
...
fix one specific thing. Nobody is refused for something a resubmission would
clear.
...
The renderer registers a TrueType font rather than using a standard PDF base
font. Helvetica's WinAnsi encoding has no glyphs for ł, ą, ę, ś, ż, ń or ğ, ş,
ı, and this document prints a real student's name. The container installs
DejaVu for that reason; transliteration is the fallback, and plain Helvetica the
last resort.
...
Addressing is by stable `id` under `/by-id/`, never by list position: the queue
is newest-first and shifts as submissions arrive.
...
`POST /reports/` accepts an optional `application_id`: an opaque key belonging
to whatever system approved the placement. This service never resolves it and
never requires it — it stores it, and `GET /reports/for-application/{id}`
returns every attempt made against it.
...
[`testdocs/tool/completion_docs.py`](../testdocs/tool/completion_docs.py)
...
diagnostic. The expected outcome for each lives in the `EXPECTED` dict in that module.
